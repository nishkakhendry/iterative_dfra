...
# Add axes image to top left corner of rendered images
...
def get_imgs(
    ...
    for key in keys:
        ...
        offset = np.array([0.0, 0.0, -0.07])  # negative Z moves the assembly up in-frame
        eye = (np.array(cam_info["eye"]) + offset).tolist()
        target = (np.array(cam_info["target"]) + offset).tolist()
        up = cam_info["up"]
...
if __name__ == "__main__":
    import matplotlib.pyplot as plt
    from prompting_and_structure.pybullet.place_blocks_in_json import (
        create_cuboid,
        init_pybullet,
        create_cylinder,
    )
    from prompting_and_structure.structure.structure import _create_cone, Block
